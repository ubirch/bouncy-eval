...
public class BoardExamples {
	...
	/**
	 * Load a RSA public key encoded in ASN.1 DER format as described by
	 * RFC5280.
	 * 
	 * @param key
	 *            The key to load as a byte array.
	 * @throws IOException
	 *             When the key is incorrect.
	 */
	public void loadPublicKeyDER(byte[] key) throws IOException {
		 ...
		 */
		RSAKeyParameters param = (RSAKeyParameters) PublicKeyFactory
				.createKey(spki);
		this.pub = param;
	}
	...
	public void loadPrivateKeyPEM(byte[] key) throws IOException {
		...
		RSAPrivateCrtKeyParameters rsa = (RSAPrivateCrtKeyParameters) PrivateKeyFactory
				.createKey(pki);
		RSAKeyParameters rsaPub = (RSAKeyParameters) PublicKeyFactory
				.createKey(spki.getEncoded());
		this.pub = rsaPub;
		this.priv = rsa;
	}

	/*
	 * public void genKey() { RSAKeyPairGenerator kpg = new
	 * RSAKeyPairGenerator(); kpg.init(new
	 * RSAKeyGenerationParameters(BigInteger.valueOf(65537), new SecureRandom(),
	 * 2048, 80)); AsymmetricCipherKeyPair keyPair = kpg.generateKeyPair();
	 * this.priv = (RSAPrivateCrtKeyParameters) keyPair.getPrivate(); this.pub =
	 * (RSAKeyParameters) keyPair.getPublic(); }
	 */

	/**
	 * Computes the SHA256 hash of a byte array and returns the hash as a byte
	 * array.
	 * 
	 * @param message
	 *            An message of arbitrary length that should be hashed.
	...
	public byte[] sign(byte[] message) throws InvalidCipherTextException {
		/*
		 * RSA Engine for the actual cryptographic unwrap operation.
		 * 
		 * The Blinded Engine provides protection against side channels.
		 */
	...
	/**
	 * Used to encrypt a session key with RSA PKCS1.5. The Session key is not
	 * allowed to exceed the maximum length of the modulus.
	 * 
	 * @param sessionKey
	 *            The session key to encrypt.
	 * @return The RSA encrypted session key using PKCS1.5 padding.
	 * @throws InvalidCipherTextException
	 */
	public byte[] encrypt(byte[] sessionKey) throws InvalidCipherTextException {
		/*
		 * Get an RSA Engine. Since this is only a pub key operation, we don't
		 * need side channel protection.
		 */
		...
		PKCS1Encoding pkcs1 = new PKCS1Encoding(rsaEngine);
		pkcs1.init(true, pub);
		return pkcs1.processBlock(sessionKey, 0, sessionKey.length);
	}

	/**
	 * Decrypt a RSA encrypted message with PKCS1.5 encoding.
	 * 
	 * @param encryptedMessage The encrypted message as byte array.
	 * @return Raw contents of the decryption with stripped padding.
	 * @throws InvalidCipherTextException When the padding is invalid.
	 */
	public byte[] decrypt(byte[] encryptedMessage)
			throws InvalidCipherTextException {
		/*
		 * Use a blinded Engine against side channel attacks.
		 */
		RSABlindedEngine rsaEngine = new RSABlindedEngine();
		/*
		 * Use PKCS#1.5 encoding.
		 */
		PKCS1Encoding pkcs1 = new PKCS1Encoding(rsaEngine);
		pkcs1.init(false, priv);
		return pkcs1.processBlock(encryptedMessage, 0, encryptedMessage.length);
	}

	/**
	 * Takes a RSA Signature with PKCS1.5 padding and returns the hash of the
	 * signed data.
	 * 
	 * @param signature
	 *            The signature to be verified.
	 * @return The Hash that was signed. No length checking of the hash is
	 *         performed.
	 * @throws InvalidCipherTextException
	 *             When the signature is malformed.
	 */
	public byte[] verifyGetHashFromSignature(byte[] signature)
			throws InvalidCipherTextException {
		/*
	...
	public static void main(String[] args) throws IOException,
		...
		byte[] toEncrypt = "This is a test".getBytes();
		try {
			byte[] encrypted = be.encrypt(toEncrypt);
			byte[] decrypted = be.decrypt(encrypted);
			System.out.println("Comparing both messages: " + Arrays.equals(decrypted, toEncrypt));
		} catch (Exception e) {
			e.printStackTrace();
		}
